cask "zoom-oci" do
  version "7.1.5.4332"
  sha256 "2c87382c0469185063fa9a3a507fea6be16882ef58c552b112297b52df3e5d18"
  ...
  name "Zoom (OCI)"
  desc "Video conferencing client running inside a rootless container"
  ...
  binary "oci/bin/zoom-oci"
  artifact "oci/share/zoom-oci.desktop",
           target: "#{Dir.home}/.local/share/applications/zoom-oci.desktop"
  artifact "usr/share/pixmaps/Zoom.png",
           target: "#{Dir.home}/.local/share/icons/hicolor/512x512/apps/zoom-oci.png"

  preflight_steps do
    run "/bin/sh",
        args:         ["-c",
                       "t='{{HOMEBREW_PREFIX}}/Homebrew/Library/Taps/oci-native/homebrew-pkgs/containers'; " \
                       "echo '==> containers/zoom-oci/Containerfile'; " \
                       "cat \"$t/zoom-oci/Containerfile\""],
        print_stdout: true
    run "/usr/bin/bsdtar", args: ["-xf", "zoom_amd64.deb", "data.tar.xz"], chdir: "."
    run "/usr/bin/bsdtar", args: ["-xf", "data.tar.xz", "./usr/share/pixmaps/Zoom.png"], chdir: "."
    mkdir_p "oci/bin"
    mkdir_p "oci/share"
    write_file "oci/bin/zoom-oci", <<~SCRIPT
      ...
      if [ -n "${OCI_NATIVE_REGISTRY:-}" ]; then
        ...
        if ! "$ENGINE" image inspect "$IMAGE" >/dev/null 2>&1; then
          ...
          DEB="{{HOMEBREW_PREFIX}}/Caskroom/$APP/$VERSION/zoom_amd64.deb"
    ...
    SCRIPT
    set_permissions "oci/bin/zoom-oci", "0755"
    write_file "oci/share/zoom-oci.desktop", <<~DESKTOP
      [Desktop Entry]
      Name=Zoom (OCI)
      Comment=Video conferencing, containerized
      Exec={{HOMEBREW_PREFIX}}/bin/zoom-oci %U
      Terminal=false
      Type=Application
      Icon=zoom-oci
      StartupWMClass=zoom
  ...
  zap trash: "~/.local/share/oci-apps/zoom-oci"
